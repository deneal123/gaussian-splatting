...
def get_scales(key, cameras, images, points3d_ordered, args):
    ...
    pts_idx = image_meta.point3D_ids  # Fixed: was using images_metas[key] which is undefined
    ...
    if valid.sum() > 10 and (invcolmapdepth.max() - invcolmapdepth.min()) > 1e-3:
        ...
        # Process in batches to avoid OpenCV's size limitation
        batch_size = 10000  # Adjust this based on your needs
        invmonodepth = np.zeros_like(invcolmapdepth)
        
        for i in range(0, len(maps), batch_size):
            batch_maps = maps[i:i+batch_size]
            batch_result = cv2.remap(
                invmonodepthmap, 
                batch_maps[..., 0], 
                batch_maps[..., 1], 
            ...
            invmonodepth[i:i+batch_size] = batch_result[..., 0]
...
if __name__ == '__main__':
    ...
    print(f"Loaded {len(images_metas)} images, {len(points3d)} points3d, {len(cam_intrinsics)} cameras")
